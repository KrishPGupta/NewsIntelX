...
│ ├── question_answering.py # Gemini-based grounded Q&A
│ └── utils.py # Shared retry logic for API calls
├── config/
│ └── settings.py # Loads API key & constants from .env
├── data/
│ └── sample_articles/ # A few example articles for quick testing
├── requirements.txt
└── README.md
```
